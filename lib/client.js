window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** When an operation changed a client-half plugin, reload the page so its UI appears/disappears. */
		function scheduleClientReload(res) {
			if (!res || res.reloadClient !== true) return;
			toast("info", "客户端已变更，1.5 秒后自动刷新页面…");
		...
		function buildManagerView(root) {
			...
			function renderExternalCard(item) {
				...
				sw.title = isSelf ? "自身保护：不可停用" : (item.enabled ? "停用" : "启用");
				if (isSelf) { sw.disabled = true; sw.style.opacity = "0.55"; }
				...
				actions.append(sw, upd);
				if (!isSelf) actions.append(rm);
				...
				if (isSelf) meta.appendChild(el("span", "dpm-tag dpm-off", "自身保护"));
				if (item.version) {
