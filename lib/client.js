		...
		function buildManagerView(root) {
			...
			function render() {
				...
				if (showOfficial) {
					...
					var shown = officialExpanded ? offFiltered : offFiltered.slice(0, 40);
					shown.forEach(function (item) { offList.appendChild(renderOfficialRow(item)); });
					if (offFiltered.length > shown.length) {
					...
					if (offFiltered.length === 0 && showExternal) {
						offList.appendChild(el("div", "dpm-empty", "无匹配。"));
					}
