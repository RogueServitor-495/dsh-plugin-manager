...
/** Does a target (patch row or bundle entry) declare a web client half? */
function targetHasClient(profileDir, target) {
  const moduleName = target
    ? target.kind === "patch"
      ? (target.row?.name ?? target.row?.id)
      : (target.entry?.options?.name ?? target.entry?.name)
    : null;
  if (!moduleName) return false;
  const dir = resolveModuleDir(profileDir, moduleName);
  const pkg = dir ? readJson(dir) : null;
  return pkg?.dsh?.client?.platform === "web" ?? false;
}
  ...
  const routes = {
    ...
    "/api/plugin-manager/toggle": async (req, res) => {
      ...
      try {
        ...
        const reloadClient = targetHasClient(info.dir, target);
        ...
        sendJson(res, 200, { ok: true, id, enabled: body.enabled, kind: target.kind, reloadClient, state: await buildState(ctx, config) });
      } catch (error) {
    ...
    "/api/plugin-manager/import": async (req, res) => {
      ...
      try {
        ...
        sendJson(res, 200, {
          ...
          reloadClient: (pkg?.dsh?.client?.platform === "web" ?? false) && !hasBundlePatch,
          bundleAdded,
          rowAdded,
    ...
    "/api/plugin-manager/update": async (req, res) => {
      ...
      try {
        ...
        sendJson(res, 200, {
          ...
          id,
          packageName,
          reloadClient: targetHasClient(info.dir, target),
          version: pkg?.version ?? null,
          output: run.output,
    ...
    "/api/plugin-manager/remove": async (req, res) => {
      ...
      try {
        ...
        if (id === name) {
          return sendJson(res, 400, { ok: false, error: { code: "SELF_REMOVE", message: "refusing to remove the plugin manager itself — edit cordis.patch.yml manually to do that" } });
        }
        const reloadClient = targetHasClient(info.dir, target);
